...
const UserTable = () => {
    ...
    const generateAndAssignWallet = async (userId, token) => {
        try {
            const response = await axios.post(`${API_BASE_URL}/admin/users/${userId}/generate-wallet`, {}, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
            console.log(`Wallet address generated and assigned for user ${userId}:`, response.data.walletAddress);
            return { userId, walletAddress: response.data.walletAddress }; // Return generated address
        } catch (err) {
            console.error(`Error generating and assigning wallet for user ${userId}:`, err.response?.data?.message || err.message);
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            const usersNeedingWallet = fetchedUsers.filter(user => !user.walletAddress);

            if (usersNeedingWallet.length > 0) {
                console.log(`Found ${usersNeedingWallet.length} users needing wallet addresses. Generating...`);
                ...
                const updatedUsers = fetchedUsers.map(user => {
                    const generated = generationResults.find(res => res.userId === user.id);
    ...
    const handleApplyTasks = async () => {
        ...
        try {
            ...
            await Promise.all(selectedUserIds.map(userId =>
                axios.put(`${API_BASE_URL}/admin/users/${userId}`, { daily_orders: parseInt(tasksToApply) }, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                })
        ...
        <div className="user-table-container">
            ...
            <div className="filters-and-apply">
                <div className="filters">
                    <input type="text" name="username" placeholder="Username" value={filters.username} onChange={handleFilterChange} />
                    <input type="text" name="phone" placeholder="Phone No" value={filters.phone} onChange={handleFilterChange} />
                    <input type="text" name="code" placeholder="Invitation Code" value={filters.code} onChange={handleFilterChange} />
                    <input type="text" name="wallet" placeholder="Wallet Address" value={filters.wallet} onChange={handleFilterChange} />
                </div>
                <div className="apply-tasks">
                    ...
                    />
                    <button onClick={handleApplyTasks} className="btn btn-green">APPLY</button>
                    {/* ADDED: Delete Selected Users Button */}
                    <button onClick={handleDeleteSelectedUsers} className="btn btn-red" style={{ marginLeft: '10px' }}>
                        Delete 
                    </button>
                </div>
            </div>
                ...
                <table className="user-table">
                    <thead>
                        <tr>
                            ...
                            }} checked={selectedUserIds.length === users.length && users.length > 0} /></th>
                            <th>ID</th>
                            <th>Username</th>
                            <th>Phone No</th>
                    ...
                    <tbody>
                        {users.length > 0 ? (
                            users.map((user) => (
                                <tr key={user.id}>
                                    <td><input type="checkbox" checked={selectedUserIds.includes(user.id)} onChange={() => handleCheckboxChange(user.id)} /></td>
                                    <td>{user.id}</td>
                                    <td>{user.username}</td>
                                    <td>{user.phone}</td>
                                    ...
                                    <td>
                                        <button className="btn btn-red" onClick={() => handleInjectClick(user.id)}>INJECT</button>
                                        <button className="btn btn-blue" onClick={() => handleSettingsClick(user)}>SETTING</button>
                                        <button className="btn btn-yellow" onClick={() => handleHistoryClick(user)}>HISTORY</button>
                                        <button className="btn btn-green" onClick={() => handleCreate(user.id)}>CREATE</button>
                                    </td>
                                </tr>
                            ))
                        ) : (
